...
import { ClearFoodHistoryUseCase, DeleteFoodHistoryItemUseCase, } from "@/features/food-history-suggestions/application/ManageFoodHistoryUseCases";
import { LocalFoodHistoryRepository } from "@/features/food-history-suggestions/infrastructure/LocalFoodHistoryRepository";
import { LocalFoodSearchProvider, LocalNutritionProvider, } from "@/features/nutrition-lookup/infrastructure/LocalNutritionProvider";
...
export const createMobileAppContainer = (userId = "local-user") => {
    ...
    const logFoodEntryUseCase = new LogFoodEntryUseCase(nutritionProvider, foodEntryRepository, foodHistoryRepository);
    const updateFoodEntryUseCase = new UpdateFoodEntryUseCase(nutritionProvider, foodEntryRepository);
    ...
    const getFoodSuggestionsUseCase = new GetFoodSuggestionsUseCase(foodSuggestionService, foodSearchProvider);
    ...
    const dailySummaryService = new LocalDailySummaryService(foodEntryRepository, userGoalRepository);
